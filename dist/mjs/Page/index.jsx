...
import { MagazineContext } from "../Context.js";
import { Geometry } from "./Geometry.js";
export const Page = ({ front, back, index, opened, current, bookClosed, ...rest }) => {
    const turnedAt = useRef(0);
    const lastOpened = useRef(opened);
    const group = useRef(null);
    const skinnedMesh = useRef(null);
    const { setCurrent } = use(MagazineContext);
    const [highlighted, setHighlighted] = useState(false);
    useCursor(highlighted);
    const [frontImage, backImage] = useTexture([front, back]);
    frontImage.colorSpace = backImage.colorSpace = SRGBColorSpace;
    const manualSkinnedMesh = useMemo(() => Geometry.createSkinnedMesh(frontImage, backImage), [frontImage, backImage]);
    useFrame((_, delta) => {
        ...
        if (Array.isArray(material) && material[4]) {
            ...
            material[5].emissiveIntensity = MathUtils.lerp(material[4].emissiveIntensity, intensity, 0.1);
        }
        ...
        for (let i = 0; i < length; i++) {
            ...
            easing.dampAngle(target.rotation, "y", turnAngle, Geometry.TURN_EASE_FACTOR, delta);
            easing.dampAngle(target.rotation, "x", foldAngle * foldIntensity, Geometry.FOLD_EASE_FACTOR, delta);
        }
    });
    const zPosition = useMemo(() => -index * Geometry.PAGE_DEPTH + current * Geometry.PAGE_DEPTH, [index, current]);
